...
class MapView extends React.Component<MapViewProps, State> {
  ...
  private handleNavigationInfoUpdated = (event: any) => {
    if (this.props.onNavigationInfoUpdated) {
      this.props.onNavigationInfoUpdated(event);
    }
  };

  private handleArrivedToDestination = (event: any) => {
    if (this.props.onArrivedToDestination) {
      this.props.onArrivedToDestination(event);
    }
  };

  private handleNavigationRouteFailedToLoad = (event: any) => {
    if (this.props.onNavigationRouteFailedToLoad) {
      this.props.onNavigationRouteFailedToLoad(event);
    }
  };

  private handleNavigationRouteLoaded = (event: any) => {
    if (this.props.onNavigationRouteLoaded) {
      this.props.onNavigationRouteLoaded(event);
    }
  };

  private handleShowRecenterButton = (event: any) => {
    if (this.props.onShowRecenterButton) {
      this.props.onShowRecenterButton(event);
    }
  };
  ...
  render() {
    ...
    const props: MapFabricNativeProps = {
      ...
      onArrivedToDestination: this.handleArrivedToDestination,
      onNavigationRouteLoaded: this.handleNavigationRouteLoaded,
      onNavigationRouteFailedToLoad: this.handleNavigationRouteFailedToLoad,
      onNavigationInfoUpdated: this.handleNavigationInfoUpdated,
      onShowRecenterButton: this.handleShowRecenterButton,
      ...restProps,
    };

...
class MapView extends React.Component {
    ...
    startNavigation(coordinate, placeId) {
        if (this.map.current) {
            MapViewNativeComponent_1.Commands.startNavigation(this.map.current, coordinate, placeId);
        }
    }
    recenter() {
        if (this.map.current) {
            MapViewNativeComponent_1.Commands.recenter(this.map.current);
        }
    }
    _mapManagerCommand(name) {
        return react_native_1.NativeModules[`${(0, decorateMapComponent_1.getNativeMapName)(this.props.navigationModeEnabled || false)}Manager`][name];
    }
    _getHandle() {
    ...
    render() {
        ...
        const AIRMap = getNativeMapComponent(this.props.navigationModeEnabled || false);
        return (<decorateMapComponent_1.ProviderContext.Provider value={'google'}>
        <AIRMap {...props}/>
...
if (react_native_1.Platform.OS === 'android') {
    airMaps.googleMapView = (0, react_native_1.requireNativeComponent)('AIRMap');
    airMaps.googleNavigationView = (0, react_native_1.requireNativeComponent)('AIRNavigationMap');
}
else {
    airMaps.googleMapView = decorateMapComponent_1.googleMapIsInstalled
...
const getNativeMapComponent = (navigationMode) => airMaps[navigationMode ? 'googleNavigationView' : 'googleMapView'];
